        ...
        for train_idx, (batch_videos, batch_keypoints) in enumerate(train_dataloader):
            ...
            for seq_idx in range(len(estimations)):
                for batch_idx in range(batch_size):
                    pose_estimations = estimations[seq_idx][batch_idx]
                    pose_estimations = pose_estimations.view(
                            pose_estimations.shape[0], 2, num_keypoints).permute(0, 2, 1)
                    classification = classifications[seq_idx][batch_idx]
                    keypoints = batch_keypoints[seq_idx][batch_idx]
            ...
            optimizer.zero_grad()
            total_loss = estimation_loss + classification_loss
            total_loss.backward()
            optimizer.step()
...
def generate_labels(pred_tensor, gt_tensor, pred_min_bounds, pred_max_bounds, gt_min_bounds, gt_max_bounds):
    ...
    print(pred_min_bounds.shape)
    print(pred_max_bounds.shape)
    print(gt_min_bounds.shape)
    print(gt_max_bounds.shape)
    matched_gt_tensor = pred_tensor.clone()
    classification_labels = torch.zeros((pred_tensor.shape[0], 1)).to(device)
