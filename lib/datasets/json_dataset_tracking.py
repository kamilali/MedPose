...
class JsonDataset(object):
    ...
    def __init__(self, name):
        ...
        if not cfg.TRAIN.POSETRACKING:
            ...
            annotations_list = [f for f in os.listdir(DATASETS[name][ANN_DIR]) if f.endswith(".json") and 'bonn' not in f]
            for annotations_file in tqdm(annotations_list):
                block_print()
                coco = COCO(os.path.join(DATASETS[name][ANN_DIR], annotations_file))
                self.cocos.append(coco)
                unblock_print()
                category_ids = coco.getCatIds()
                categories = [c['name'] for c  in coco.loadCats(category_ids) if c['name'] not in self.classes]

                curr_category_to_id_map = dict(zip(categories, category_ids))
                self.category_to_id_map.update(curr_category_to_id_map)

                self.classes = self.classes + categories

                curr_json_category_id_to_contiguous_id = {
                    v: i + 1
                    for i, v in enumerate(coco.getCatIds())
                }
                curr_contiguous_category_id_to_json_id = {
                    v: k
                    for k, v in curr_json_category_id_to_contiguous_id.items()
                }
                self.json_category_id_to_contiguous_id.update(curr_json_category_id_to_contiguous_id)
                self.contiguous_category_id_to_json_id.update(curr_contiguous_category_id_to_json_id)

            self._init_keypoints(coco)
            self.num_classes = len(self.classes)

            print(len(self.cocos))
    ...
    def get_roidb(
        ...
        # roidb for each video
        roidbs = []
        # iterate through coco objects for each video
        print("getting roidbs")
        for coco in self.cocos:
            image_ids = coco.getImgIds()
            image_ids.sort()
            if cfg.DEBUG:
                roidb = copy.deepcopy(coco.loadImgs(image_ids))[:1]
            else:
    ...
    def _init_keypoints(self, coco=None):
        ...
        if 'person' in self.category_to_id_map:
            cat_info = coco.loadCats([self.category_to_id_map['person']])
        else:
            return
...
# utils to supress prints for coco loads
def block_print():
