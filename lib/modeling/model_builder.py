...
import modeling.medpose as medpose
...
class Generalized_RCNN(nn.Module):
    def __init__(self):
        ...
        if cfg.MODEL.MEDPOSE_ON:
            self.med_pose_network = medpose.med_pose_network(self.roi_feature_transform, self.Conv_Body.spatial_scale)
    ...
    def forward(self, data, im_info, roidb=None, **rpn_kwargs):
        ...
        if not cfg.MODEL.RPN_ONLY:
            if cfg.MODEL.SHARE_RES5 and self.training:
                ...
                box_feat, roi_feats = self.Box_Head(blob_conv, rpn_ret)
            cls_score, bbox_pred = self.Box_Outs(box_feat)
            #print(box_feat.shape, 'box feat shape')
            #print(cls_score.shape, 'cls shape')
            #print(bbox_pred.shape, 'bbox shape')
        ...
        if self.training:
            ...
            if cfg.MODEL.MEDPOSE_ON:
                #print("This is going to be VERY PAINFUL")
                # print(box_feat.shape, "box features shape")
                # print(cls_score.shape, "cls score shape")
                # print(bbox_pred.shape, "box pred shape")
                # print(blob_conv[0].shape, "get keypoints blob conv")
                # print(rois.shape, "get rois from faster rcnn to be used for keypoints")
                
                # for now, pose estimation is based only on the largest feature map
                # for key in rpn_ret.keys():
                #     print(key, rpn_ret[key].shape)

                # new strategy: produced keypoint heatmaps
                heat_out, cl_out = self.med_pose_network(blob_conv, rpn_ret)
                # old strategy: keypoint regression
                #reg_out, cl_out = self.med_pose_network(blob_conv, rpn_ret)
                
                ################ THIS OUTPUTS DATA SIMILAR TO WHAT WE SAW IN FIRST IMPL #################
                #cl_out, reg_out = self.med_pose_network(blob_conv[len(blob_conv) - 1], roi_feats, rois)
                
                #for i in range(blob_conv[0].shape[0]):
                #    print("For", i, "we have this many regions:")
                #    print(rois[(rois[:,0] == i).nonzero().squeeze(1)].shape)
                #print(cl_out.shape, reg_out.shape)
                #print(rpn_ret['keypoint_locations_int32'].shape, rpn_ret['keypoint_weights'].shape, rpn_ret['keypoint_loss_normalizer'].shape)
                #print(rpn_ret['keypoint_locations_int32'].shape, "just testing keypoint labels###############################")
                #print(rpn_ret['keypoint_locations_vec'].shape)
                #print(rpn_ret['keypoint_weights'])
                #print(rpn_ret['keypoint_loss_normalizer'])
                if cfg.KRCNN.NORMALIZE_BY_VISIBLE_KEYPOINTS:
                    # old strategy: keypoint regression
                    #loss_keypoints = medpose.keypoint_losses(reg_out, rpn_ret['keypoint_rois'], rpn_ret['keypoint_locations_vec'], rpn_ret['keypoint_locations_int32'], rpn_ret['keypoint_weights'])
                    loss_keypoints = medpose.keypoint_losses(heat_out, rpn_ret['keypoint_locations_int32'], rpn_ret['keypoint_weights'])
                else:
                    # old strategy: keypoint regression
                    #loss_keypoints = keypoint_rcnn_heads.keypoint_losses(
                    #    reg_out, rpn_ret['keypoint_rois'], rpn_ret['keypoint_locations_vec'], rpn_ret['keypoint_locations_int32'], rpn_ret['keypoint_weights'], rpn_ret['keypoint_loss_normalizer'])
                    loss_keypoints = medpose.keypoint_losses(heat_out, rpn_ret['keypoint_locations_int32'], rpn_ret['keypoint_weights'], rpn_ret['keypoint_loss_normalizer'])
                print(loss_keypoints, flush=True)
    ...
    def detectron_weight_mapping(self):
        if self.mapping_to_detectron is None:
            ...
            for name, m_child in self.named_children():
                # med pose network is not a part of the original detectron network
                #if name != cfg.MEDPOSE.MODULE_NAME:
