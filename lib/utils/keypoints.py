...
def keypoints_to_pred_heatmap(keypoints, rois, visibilities):
    """Encode keypoint location in the prediction heatmap for use in
    SoftmaxWithLoss.
    """
    # Maps keypoints from the half-open interval [x1, x2) on continuous image
    # coordinates to the closed interval [0, HEATMAP_SIZE - 1] on discrete image
    ...
    device_id = keypoints.get_device()
    rois = Variable(torch.from_numpy(rois)).cuda(device_id)
    visibilities = Variable(torch.from_numpy(visibilities)).cuda(device_id)

    shape = (len(rois), cfg.KRCNN.NUM_KEYPOINTS)
    heatmaps = Variable(torch.zeros(shape).float())
    ...
    # old
    for kp in range(keypoints.shape[2]):
        vis = (visibilities[:, kp] > 0)
        x = keypoints[:, 0, kp].float()
        y = keypoints[:, 1, kp].float()
        # Since we use floor below, if a keypoint is exactly on the roi's right
        # or bottom boundary, we shift it in by eps (conceptually) to keep it in
        # the ground truth heatmap.
        x_boundary_inds = (x == rois[:, 2]).nonzero()
        y_boundary_inds = (y == rois[:, 3]).nonzero()
        #x_boundary_inds = torch.where(x == rois[:, 2])[0]
        #y_boundary_inds = torch.where(y == rois[:, 3])[0]
        x = (x - offset_x) * scale_x
        x = torch.floor(x)
        if len(x_boundary_inds) > 0:
            x[x_boundary_inds] = cfg.KRCNN.HEATMAP_SIZE - 1
        ...
        valid_loc = ((x >= 0) & (y >= 0)) & ((x < cfg.KRCNN.HEATMAP_SIZE) & (y < cfg.KRCNN.HEATMAP_SIZE))
        valid = valid_loc & vis
        valid = valid.float()
